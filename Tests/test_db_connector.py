...
#using @patch to works with mock (fake object to avoid connecting to the true DB)
...
class TestQueries(unittest.TestCase):
    ...
    def test_all_queries_are_strings(self):
        query_names = [
            ...
            "CREATE_CUSTOMER", "GET_CUSTOMERS", "LIST_CUSTOMERS",
    ...
    def test_insert_queries_have_placeholders(self):
        ...
        for q in insert_queries:
            with self.subTest(query=q[:40]):   #subtest continue even if fail.  q40 to limit to 40char for the display of the failed query
                self.assertIn("%s", q)
